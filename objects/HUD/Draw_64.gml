...
draw_set_halign(fa_center);
draw_set_valign(fa_middle);
draw_text(_coolX + (_coolW * 0.5), _subY + (_coolH * 0.5), "SP");
...
for (var j = 0; j < _burstMax; j++) {
	...
	// player number above head in world space — convert to GUI coords
	...
	var _screenY   = (_pl.y - _camY - 40) * _guiScaleY; // 40px above player
	draw_set_color(c_white);
	draw_set_halign(fa_center);
	draw_set_valign(fa_middle);
	draw_set_font(-1);
